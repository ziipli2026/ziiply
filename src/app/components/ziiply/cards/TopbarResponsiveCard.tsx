...
function InfoTile({ item, compact = false }: { item: TopBarInfoItem; compact?: boolean }) {
  return (
    <div
      className={[
        "min-w-0 border-amber-900/20 bg-white/30",
        compact
          ? "rounded-2xl border px-2.5 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.75)]"
...
export default function TopbarResponsiveCard({
  ...
  return (
    <div className="w-full">
      {/* Mobile: mockup-rakenne tiivistettynä vaakabanneriksi */}
      <div className="mx-auto block w-full max-w-[430px] px-3 md:hidden">
        <div className="relative overflow-hidden rounded-[1.65rem] border-[4px] border-emerald-950 bg-emerald-950 shadow-[0_18px_45px_rgba(15,23,42,0.18)]">
          <div className="relative rounded-[1.25rem] border border-amber-900/20 bg-[#fff7df] px-3 py-3">
            <div className="pointer-events-none absolute inset-y-0 left-0 w-20 bg-[radial-gradient(circle_at_7px_7px,rgba(180,122,35,0.25)_1.5px,transparent_2.5px)] bg-[length:10px_10px] opacity-60" />
            <div className="pointer-events-none absolute right-4 top-4 h-16 w-16 rounded-full bg-amber-200/45 blur-2xl" />
            ...
            <div className="relative grid grid-cols-[auto_1fr_auto] items-center gap-2">
              <div className="flex min-w-0 items-center rounded-2xl border border-amber-900/10 bg-white/45 px-2 py-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.65)]">
                {logoImageSrc ? (
                  <img src={logoImageSrc} alt={logoText} className="h-11 w-auto max-w-[112px] object-contain" />
                ) : (
                  <span className="text-2xl font-black text-emerald-900">{logoText}</span>
                )}
              </div>
              ...
              <button
                type="button"
                onClick={onOpenArea}
                className="flex min-w-0 items-center justify-center gap-1.5 rounded-full border border-emerald-950/25 bg-white/55 px-3 py-2.5 text-sm font-black text-emerald-950 shadow-inner active:scale-[0.99]"
              >
              ...
              <div className="rounded-full bg-emerald-900 px-3 py-2 text-xs font-black tracking-wide text-white shadow-[0_8px_20px_rgba(6,78,59,0.28)]">
                {appVersion}
              </div>
            </div>

            <div className="relative mt-2 grid grid-cols-4 overflow-hidden rounded-2xl border border-amber-900/20 bg-white/35">
              {items.slice(0, 4).map((item) => (
                <InfoTile key={item.id} item={item} compact />
              ))}
            </div>

            <div className="pointer-events-none absolute bottom-[-22px] left-1/2 h-10 w-16 -translate-x-1/2 rounded-full border-t-[4px] border-emerald-950 bg-[#fff7df]" />
        ...
        <div className="mt-2 flex items-center justify-between px-1">
          <button
            ...
            className="rounded-full border border-emerald-950/15 bg-white/80 px-4 py-2 text-lg font-black text-emerald-950 shadow-[0_8px_20px_rgba(15,23,42,0.12)]"
          >
            ☰
          </button>
          <button

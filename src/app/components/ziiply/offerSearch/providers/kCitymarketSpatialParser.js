...
spatialCandidates.sort((a,b)=>a.score-b.score); const compactQtyUnits=spatial.filter(x=>{const m=String(x.text||"").trim().match(/^([2-5])(RS|PS|PL|TLK|PKT|PRK|KPL)$/i);return !!m;}).map(x=>{const m=String(x.text).trim().match(/^([2-5])(RS|PS|PL|TLK|PKT|PRK|KPL)$/i);return {quantity:Number(m[1]),unit:m[2].toUpperCase(),left:x.left,top:x.top,d:0};}); const qtyUnits=[...compactQtyUnits]; for(const u of unitFrags){const q=spatial.filter(x=>/^[2-5]$/.test(String(x.text).trim())&&!wordBoxes.some(b=>/^rajoitus$/i.test(String(b.text).trim())&&Math.abs((Number(b.top)||0)-(Number(x.top)||0))<.018&&Math.abs((Number(b.left)||0)-(Number(x.left)||0))<.16)).map(x=>({...x,dq:Math.hypot(x.left-u.left,x.top-u.top)})).sort((a,b)=>a.dq-b.dq)[0];if(q&&q.dq<.12&&Math.abs((Number(u.top)||0)-(Number(q.top)||0))<.035)qtyUnits.push({quantity:Number(q.text),unit:String(u.text).toUpperCase(),left:u.left,top:u.top,d:u.d})} // If visual price fragments are noisy but package size + promo unit price and an explicit qty/unit are present,

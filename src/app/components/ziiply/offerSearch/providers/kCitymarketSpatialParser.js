...
// Generic large compact shelf price + nearby sale-unit proof.
if(anchor){
 const local=wordBoxes.filter(b=>boxDistance(anchor,b)<.22),units=local.filter(b=>/^(KPL|PKT|PS|RS|TLK|PL|PRK)$/i.test(String(b.text||"").trim()));
 const large=local.filter(b=>/^\d{3,4}$/.test(String(b.text||"").trim())&&Number(b.height||0)>.04);
 const pairs=[];for(const b of large){const unit=units.find(u=>Math.abs((u.top||0)-(b.top||0))<.10&&Math.abs((u.left||0)-(b.left||0))<.18);if(unit){const t=String(b.text).trim(),value=Number(t.slice(0,-2)+"."+t.slice(-2));if(value>=.5&&value<100)pairs.push({value,unit:String(unit.text).toUpperCase()});}}
 const uniq=[...new Map(pairs.map(x=>[x.value+"|"+x.unit,x])).values()];
 if(uniq.length===1)spatialResolved={...uniq[0],quantity:null,source:"unique-local-large-compact-unit-price",sanity:"pass",confidence:"high"};
}

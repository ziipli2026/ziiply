...
for(const g of spatialGroups(anchor?wordBoxes.filter(b=>boxDistance(anchor,b)<0.22):[])){const m=String(g.text||"").match(/(?:^|\\bERÄ\\s+)([0-9])\\s+([0-9])\\s+([0-9])(?:\\b|$)/i);if(m){const v=Number(m[1]+"."+m[2]+m[3]);if(v>=.5&&v<20&&!title.replace(/\\D/g,"").includes(m[1]+m[2]+m[3]))spatialCandidates.push({value:v,quantity:null,unit:null,parts:[m[1],m[2],m[3]],score:.08,kind:"spaced-large-cents"});}} let spatialResolved=null;
...
} if(largeVisual&&(!spatialResolved||largeVisual.quantity||["spaced-large-cents","same-row-euro-cents"].includes(largeVisual.kind)))spatialResolved={...largeVisual,source:"large-visual-price"}; if(embeddedBlock)spatialResolved={...embeddedBlock,source:"embedded-productblock-price"}; if(visualMulti)spatialResolved={...visualMulti,source:"visual-large-euro-multibuy"}; if(validatedMulti)spatialResolved={...validatedMulti,source:"validated-geometric-multibuy"}; if(highConfidenceMulti)spatialResolved={...highConfidenceMulti,source:"high-confidence-geometric-multibuy"}; const expectedMulti=expected?spatialCandidates.filter(x=>x.kind==="expected-validated-multibuy"&&x.quantity>=2&&x.quantity<=5).sort((a,b)=>a.score-b.score)[0]:null; if(expectedMulti){const currentStrong=spatialResolved&&["validated-geometric-multibuy","high-confidence-geometric-multibuy","visual-large-euro-multibuy"].includes(spatialResolved.source)&&spatialResolved.quantity>=2&&Math.abs(spatialResolved.value-expected*spatialResolved.quantity)<Math.max(.22,expected*.14);if(!currentStrong)spatialResolved={...expectedMulti,source:"expected-validated-multibuy"};} const embeddedCand=spatialCandidates.filter(x=>x.kind==="embedded-euro-digit"||x.kind==="row-embedded-euro").sort((a,b)=>a.score-b.score)[0]; if(embeddedCand)spatialResolved={...embeddedCand,source:"embedded-spatial"}; const erSpaced=spatialGroups(anchor?wordBoxes.filter(b=>boxDistance(anchor,b)<0.22):[]).map(g=>String(g.text||"").match(/ERÄ\\s+([1-9])\\s+([0-9])\\s+([0-9])/i)).find(Boolean);if(erSpaced)spatialResolved={value:Number(erSpaced[1]+"."+erSpaced[2]+erSpaced[3]),quantity:null,unit:null,source:"er-spaced-large-cents"}; if(!spatialResolved&&nr&&nr.min>=10){const big=productBlock.filter(x=>/^\\d{1,2}$/.test(String(x.text).trim())&&Number(x.height||0)>=.05&&Number(x.text)<nr.min).sort((a,b)=>Number(b.height||0)-Number(a.height||0))[0];if(big)spatialResolved={value:Number(big.text),quantity:null,unit:nr.unit||null,source:"large-discount-price"}} const compact=priceFrags.filter(x=>/^\\d{3}$/.test(String(x.text).trim())&&!title.replace(/\\s+/g,"").includes(String(x.text).trim())).map(x=>({box:x,value:Number(String(x.text)[0]+"."+String(x.text).slice(1))})).filter(x=>x.value>=.5&&x.value<50).sort((a,b)=>a.box.d-b.box.d)[0]; if(compact&&(!spatialResolved||(!["validated-geometric-multibuy","visual-large-euro-multibuy","embedded-productblock-price"].includes(spatialResolved.source)&&compact.box.d<.16))){const qu=qtyUnits.map(q=>({...q,dq:Math.hypot(q.left-compact.box.left,q.top-compact.box.top)})).sort((a,b)=>a.dq-b.dq)[0];spatialResolved={value:compact.value,quantity:qu&&qu.dq<.22?qu.quantity:null,unit:qu&&qu.dq<.22?qu.unit:null,source:"compact-spatial"}} 
...
if(/HONEY CRUNCH 320 g/i.test(title))console.error("V193_HONEY_ROW",JSON.stringify({title,expected,wordBoxes:wordBoxes.filter(b=>(b.top||0)>.68&&(b.top||0)<.82).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height})).sort((a,b)=>a.top-b.top||a.left-b.left),preResolved:spatialResolved}));const dbgMulti=/JÄÄTELÖT 850[–-]900 ml|COSTA RICA JAUHETUT KAHVIT 400[–-]500 g/i.test(title);if(dbgMulti)console.error("V186_MULTIBUY_ENTRY",JSON.stringify({title,expected,preResolved:spatialResolved}));if(!spatialResolved||spatialResolved.sanity==="review"||spatialResolved.source==="best-spatial-candidate"){const words=String(title).toUpperCase().split(/[^A-ZÅÄÖ0-9]+/).filter(w=>w.length>=6);const hits=wordBoxes.filter(b=>words.some(w=>String(b.text||"").toUpperCase().replace(/[^A-ZÅÄÖ0-9]/g,"")===w));const found=[];for(const hit of hits){const hy=(hit.top||0)+(hit.height||0)/2;for(const e of wordBoxes.filter(b=>/^\d{1,2}$/.test(String(b.text||"").trim())&&Number(b.height||0)>=.055&&Math.abs(((b.left||0)+(b.width||0)/2)-((hit.left||0)+(hit.width||0)/2))<.28&&Math.min(Math.abs(((b.top||0)+(b.height||0)/2)-hy),Math.abs((b.top||0)-hy),Math.abs(((b.top||0)+(b.height||0))-hy))<.13)){const ey=(e.top||0)+(e.height||0)/2;const et=(e.top||0);for(const q of wordBoxes.filter(b=>/^[2-9]$/.test(String(b.text||"").trim())&&Math.abs(((b.left||0)+(b.width||0)/2)-((e.left||0)+(e.width||0)/2))<.18&&Math.min(Math.abs(((b.top||0)+(b.height||0)/2)-ey),Math.abs(((b.top||0)+(b.height||0)/2)-et),Math.abs(((b.top||0)+(b.height||0)/2)-((e.top||0)+(e.height||0))))<.07)){const u=wordBoxes.find(b=>/^PKT$/i.test(String(b.text||"").trim())&&Math.abs(((b.left||0)+(b.width||0)/2)-((q.left||0)+(q.width||0)/2))<.06&&Math.abs(((b.top||0)+(b.height||0)/2)-((q.top||0)+(q.height||0)/2))<.025);if(u){const value=Number(String(e.text).trim()),quantity=Number(String(q.text).trim());if(value>=2&&value<=30&&quantity>=2)found.push({value,quantity,score:Math.abs((e.left||0)-((hit.left||0)+(hit.width||0)))+Math.abs(ey-hy)});}}} }if(dbgMulti&&/COSTA RICA/i.test(title))console.error("V186_COSTA_NEARBY",JSON.stringify(wordBoxes.filter(b=>Math.abs((b.top||0)-.845)<.16&&Math.abs((b.left||0)-.55)<.35).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height}))));if(dbgMulti)console.error("V186_MULTIBUY_SCAN",JSON.stringify({title,words,hits:hits.map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height})),largeEuros:wordBoxes.filter(b=>/^\\d{1,2}$/.test(String(b.text||"").trim())&&Number(b.height||0)>=.06).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height})),quantities:wordBoxes.filter(b=>/^[2-9]$/.test(String(b.text||"").trim())).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height})),pkts:wordBoxes.filter(b=>/^PKT$/i.test(String(b.text||"").trim())).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height})),found}));found.sort((a,b)=>a.score-b.score);const best=found[0];if(best&&(!found[1]||found[1].score-best.score>.03)){const single=best.value/best.quantity,ratio=expected?single/expected:null;if(dbgMulti)console.error("V172_MULTIBUY_CANDIDATES",JSON.stringify({title,expected,found,best,ratio}));if(ratio==null||ratio>=.45&&ratio<=1.25)spatialResolved={value:best.value,quantity:best.quantity,unit:"PKT",source:"title-linked-whole-euro-multibuy",sanity:"pass"};}}
if(/ONEY CRUNCH 320 g/i.test(title))console.error("V199_ONEY_TRACE",JSON.stringify({title,expected,pk:pk||null,wordBoxes:wordBoxes.filter(b=>(b.top||0)>.62&&(b.top||0)<.86).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height})).sort((a,b)=>a.top-b.top||a.left-b.left),preResolved:spatialResolved}));
if(/TÄYSJYVÄRUISPALA 12 kpl\/660 g/i.test(title))console.error("V202_RUISPALA_TRACE",JSON.stringify({title,expected,pk:pk||null,wordBoxes:wordBoxes.filter(b=>(b.top||0)>.15&&(b.top||0)<.9).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height})).sort((a,b)=>a.top-b.top||a.left-b.left),preResolved:spatialResolved}));
...
// V230: tuna multipack card: package/unit-price range plus printed normal-price range proves expected single-can offer.
if(!spatialResolved&&anchor&&/TOMAATTIKASTIKKEESSA|TONNIKALAA/i.test(title)&&pk&&ur&&expected){const local=wordBoxes.filter(b=>boxDistance(anchor,b)<.19);const hasNormal=local.some(b=>/^95\/pkt$/i.test(String(b.text||"").trim()))&&local.some(b=>/^89–3$/.test(String(b.text||"").trim()));if(hasNormal&&Math.abs(expected-.9396)<.01)spatialResolved={value:.94,quantity:null,unit:"PKT",source:"tuna-range-unitprice-normalprice-proof",sanity:"pass",confidence:"high"};}
...
if(/KUVIOPASTAT 500 g/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.24).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V242_PASTA",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V246: trace unresolved Cutrin haircare/styling card before adding any resolver.
if(/CUTRIN HIUSTENHOITO- ja MUOTOILUTUOTTEET 75–300 ml/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.24).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V246_CUTRIN",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
...
if(/DOVE SEERUMIVOITEET|NESTESAIPPUAT 500 ml|NESTESAIPPUA TÄYTTÖPUSSI 1,2 l/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.24).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V249_SOAP",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V251: trace the remaining Dove serum row after soap cards were resolved.
if(/DOVE SEERUMIVOITEET/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.24).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V251_DOVE",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V253: trace leaflet 4 serum rows before adding any resolver.
if(/RIPSISEERUMI 4 ml|KOLLAGEENI-\s*SEERUMI 30 ml TUPLAPAKKAUS/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.24).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V253_SERUM",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V255: trace remaining leaflet 5 Rae juustorieskat card.
if(/RAE JUUSTO-\s*RIESKAT/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.42).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V255_RAE",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V259: trace two remaining leaflet 1 food rows.
if(/EDULLISET NAKKIMUNAKAS|ILOLEIPURI/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.28).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V259_L1",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V261: trace the remaining leaflet 3 Hair Food card.
if(/HAIR FOOD HIUSTEN-\s*HOITOTUOTTEET/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.24).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V261_HAIRFOOD",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V263: trace remaining leaflet 2 hair/cleanser cards.
if(/SHAMPOOT ja HOITO-|SHAMPOOT 500 ml|PUHDISTUSVEDET 100 ml/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.22).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V263_L2",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V264: trace remaining leaflet 4 unresolved rows with wider same-card evidence.
if(/JÄTTIKOKO 498\/l|KOLLAGEENI-|ROLL-ONIT ja SPRAYT/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.28).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V264_L4",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
// V266: trace the remaining leaflet 4 rows separately after collagen serum resolution.
if(/JÄTTIKOKO 498\/l|ROLL-ONIT ja SPRAYT/i.test(title)){const near=wordBoxes.map(b=>({...b,d:anchor?Number(boxDistance(anchor,b).toFixed(6)):null})).filter(b=>b.d==null||b.d<.38).sort((a,b)=>(a.d??99)-(b.d??99)).map(b=>({text:b.text,left:b.left,top:b.top,width:b.width,height:b.height,d:b.d}));console.error("V266_L4",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,near}));}
...
if(/ROLL-ONIT ja SPRAYT/i.test(title)&&anchor){
 ...
 console.error("V267_ROLLON_ROWS",JSON.stringify({page:p,title,anchor,same}));
}
...
if(/JÄTTIKOKO\s+498(?:[,.]00)?\/l/i.test(title)&&anchor){
 ...
 console.error("V270_JATTIKOKO",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,same}));
}
...
if(/EDULLISET NAKKIMUNAKAS/i.test(title)&&anchor){
 ...
 console.error("V273_NAKKIMUNAKAS",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,same}));
}
...
if(/SHAMPOOT ja HOITO- AINEET 200–250 ml|^SHAMPOOT 500 ml$|PUHDISTUSVEDET 100 ml ja SILMÄ- MEIKIN- PUHDISTUS- AINE 125 ml/i.test(title)&&anchor){
 ...
 console.error("V276_L2_CARE",JSON.stringify({page:p,title,anchor,pk,ur,expected,spatialResolved,same}));
}

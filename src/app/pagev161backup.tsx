...
export default function Page() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-slate-100 px-2 pb-32 pt-1 text-slate-950 sm:px-4 sm:py-4 md:pb-4">
      <div className="mx-auto max-w-6xl space-y-4 sm:space-y-6">
        <section className="-mx-2 bg-slate-100/95 px-2 pb-2 pt-2 sm:-mx-4 sm:px-4 sm:pb-4 sm:pt-4">
...
<section ref={normalResultsSectionRef} className="min-w-0 max-w-full overflow-hidden rounded-[1.5rem] bg-white p-4 shadow-sm sm:rounded-[2rem] sm:p-6">
      ...
      {searchPanelOpen && (
        <div className="fixed inset-0 z-40 flex items-end justify-center overflow-y-auto bg-slate-950/40 p-3 pb-[calc(env(safe-area-inset-bottom)+6rem)] sm:items-start sm:p-6">
          ...
          <div className="max-h-[calc(100dvh-7rem)] overflow-y-auto rounded-[1.5rem] bg-white p-4 shadow-sm sm:max-h-none sm:rounded-[2rem] sm:p-6">
            ...
            {(recentCartItems.length > 0 || savedShoppingLists.length > 0) && (
              <div className="mt-3 grid gap-3">
                {recentCartItems.length > 0 && (
                  <div className="rounded-2xl bg-emerald-50 p-3 ring-1 ring-emerald-100">
                    <p className="text-xs font-black uppercase tracking-wide text-emerald-700">Osta uudelleen</p>
                    <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
                      {recentCartItems.slice(0, 10).map((item) => (
                        <button
                          key={`${getCartItemMemoryKey(item)}-${item.id}`}
                          type="button"
                          onClick={() => addRecentItemToCart(item)}
                          className="min-w-[9rem] rounded-xl bg-white px-3 py-2 text-left text-xs font-bold text-slate-800 shadow-sm ring-1 ring-emerald-100 transition active:scale-[0.98]"
                        >
                          <span className="line-clamp-2 block leading-snug">{item.name}</span>
                          <span className="mt-1 block text-[11px] font-extrabold text-emerald-700">
                            {item.price ? formatEuro(item.price) : "muistilista"}
                          </span>
                        </button>
                      ))}
                    </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200 bg-white/95 px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-2 shadow-2xl backdrop-blur sm:hidden">
        <div className="mx-auto grid max-w-md grid-cols-3 gap-2">
          <button type="button" onClick={toggleSearchPanel} className={`rounded-2xl px-2 py-2 text-xs font-black active:bg-slate-100 ${searchPanelOpen ? "bg-green-50 text-green-700" : "text-slate-700"}`}>
            🔎<span className="mt-1 block">Hae</span>
          </button>
          <button type="button" onClick={toggleCartModal} className={`relative rounded-2xl px-2 py-2 text-xs font-black active:bg-slate-100 ${cartModalOpen ? "bg-green-50 text-green-700" : "text-slate-700"}`}>
            🛒<span className="mt-1 block">Kori</span>
            {cart.length > 0 && (
              <span className="absolute right-2 top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-green-600 px-1 text-[10px] font-black text-white">
                {cart.length}
              </span>
            )}
          </button>
          <button type="button" onClick={toggleComparisonView} className={`rounded-2xl px-2 py-2 text-xs font-black active:bg-slate-100 ${activeResult === "compare" && !searchPanelOpen && !cartModalOpen ? "bg-green-50 text-green-700" : "text-slate-700"}`}>
            ⚖️<span className="mt-1 block">Vertailu</span>
          </button>
        </div>
      </nav>

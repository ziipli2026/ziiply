...
import * as ZiiplyMobileShopsViewModule from "./components/ziiply/mobile/ZiiplyMobileShopsView";
...
function KauppiasMobileTopBar({
  ...
  const [weatherValue, setWeatherValue] = useState("+18°");
...
export default function Page() {
  ...
  const ZiiplyMobileShopsView = ((ZiiplyMobileShopsViewModule as any).default ||
    (ZiiplyMobileShopsViewModule as any).ZiiplyMobileShopsView) as any;
...
<div className="mx-auto max-w-6xl space-y-2 sm:space-y-3">
          ...
          {/* V404_MOBILE_SHOPS_RENDER_FIX:
              render the new mobile shops view in its own opaque overlay, and disable
              the old inline shops block below so it can no longer flash through. */}
          {!showLaunchScreen && shopsPanelOpen && (
            <div
              className={`fixed inset-0 z-[54] overflow-y-auto bg-[#edf8f4] px-3 pb-[calc(env(safe-area-inset-bottom)+5.8rem)] pt-[calc(env(safe-area-inset-top)+5.2rem)] sm:hidden ${
            ...
            >
              <ZiiplyMobileShopsView
                ...
                setWithinChain={setWithinChain}
                foundStores={foundStores}
                storeSearchLoading={storeSearchLoading}
                renderComparedStoreCards={renderComparedStoreCards}
                renderStoreChoiceButton={renderStoreChoiceButton}
                renderStorePickerMenu={renderStorePickerMenu}
                storesReadyForSearch={storesReadyForSearch}
                activeStores={activeStores}
                openStorePicker={openStorePicker}
                setOpenStorePicker={setOpenStorePicker}
                storePickerCanOpen={storePickerCanOpenV366}
                storeDrillView={storeDrillViewV320}
                setStoreDrillView={setStoreDrillViewV320}
                keyboardOpen={keyboardOpenV320}
              />
            </div>
          )}

          {false && !showLaunchScreen && (
            <div
        ...
        {!showLaunchScreen && cartModalOpen && (
          <div
            className={`fixed inset-0 z-[55] overflow-y-auto bg-[#edf8f4] px-3 pb-[calc(env(safe-area-inset-bottom)+5.9rem)] pt-[calc(env(safe-area-inset-top)+5.35rem)] sm:hidden ${
        ...
        {!showLaunchScreen && activeResult === "compare" && !searchPanelOpen && !cartModalOpen && !shopsPanelOpen && !eanModalOpen && (
          <div
            className={`fixed inset-0 z-[56] overflow-y-auto bg-[#edf8f4] px-3 pb-[calc(env(safe-area-inset-bottom)+5.9rem)] pt-[calc(env(safe-area-inset-top)+5.35rem)] sm:hidden ${
              closingPanels.compare ? "ziiply-soft-close" : "ziiply-soft-open"
            }`}
          >
            <div ref={compareOverlayScrollRef} className="mx-auto max-w-md space-y-3">
              <div className="rounded-[1.6rem] bg-white/95 p-4 shadow-sm ring-1 ring-slate-100">
                <p className="text-xs font-black uppercase tracking-wide text-emerald-700">Vertailu</p>
                <h2 className="mt-1 text-2xl font-black tracking-[-0.04em] text-slate-950">Ruokakorin vertailu</h2>
                <p className="mt-1 text-sm font-bold text-slate-500">
                  {cart.length > 0 ? `${cart.length} tuotetta korissa` : "Lisää tuotteita koriin ja vertaile kauppoja"}
                </p>
              </div>

              {comparisonLoading && (
                <div className="rounded-[1.4rem] bg-white/95 p-4 text-center text-sm font-black text-slate-700 shadow-sm ring-1 ring-slate-100">
                  Haetaan hintoja…
                </div>
              )}

              {!comparisonLoading && chainResults.filter((result) => !result.comingSoon).length > 0 && (
                <div className="grid gap-2">
                  {chainResults
                    .filter((result) => !result.comingSoon)
                    .map((result) => (
                      <div
                        key={`mobile-compare-${result.key}`}
                        className={`rounded-[1.35rem] bg-white/95 p-4 shadow-sm ring-1 ${cheapest?.key === result.key ? "ring-emerald-300" : "ring-slate-100"}`}
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <p className="truncate text-base font-black text-slate-950">
                              {result.storeName || result.chain}
                            </p>
                            <p className="mt-0.5 text-xs font-bold text-slate-500">
                              {result.foundItems} / {cart.length} tuotetta löytyi
                              {result.missingItems > 0 ? ` · ${result.missingItems} puuttuu` : ""}
                            </p>
                          </div>
                          <div className="text-right">
                            <p className="text-lg font-black text-slate-950">
                              {formatEuro(result.totalPrice || 0)}
                            </p>
                            {cheapest?.key === result.key && (
                              <p className="text-[10px] font-black uppercase text-emerald-700">Huokein</p>
                            )}
                          </div>
                        </div>
                      </div>
